...
<script type="text/javascript">
	...
	function searchFunction(str){
		showloading();
		load('mhs/simkrs_ksp/cari_matakuliah/'+str,'#nama_mk');
		stoploading();
	}
</script>
...
<?php if($sudah_krs){ ?>
	<a href="javascript:void(0)" onclick='show("mhs/simkrs_ksp/cetak_krs","#center-column")' class='print-button'>Preview KRS</a>
<?php
		}
	}
?>
...
<?php echo $this->pquery->form_remote_tag(array(
	'url'=>site_url('mhs/simkrs_ksp/detail_mahasiswa'), 							
...
<?php
	...
	'url'=>site_url('mhs/simkrs_ksp/simpan'), 'update'=>'#center-column', 'name'=>'pilih', 'id'=>'form', 'type'=>'post'));
?>
	...
	<input type='radio' name='status' checked value='mengulang'> Mengulang
</form>
<?php
	echo $this->pquery->form_remote_tag(array(
	'url'=>site_url('mhs/simkrs_ksp/simpan_tabel'), 'update'=>'#center-column', 'name'=>'pilih', 'id'=>'form', 'type'=>'post'));
?>
...
<?php $i=1; $jum = 0; $no=1; foreach($this->cart->contents() as $items): ?>
	...
	<tr class="bg">
	  ...
	  <td>
		<a href="javascript:void(0)" onclick='show("mhs/simkrs_ksp/delete/<?php echo $items['rowid']?>","#center-column")'>
...
<div class='panel'>
	...
	<?php
		if($nama_dpa == false){
			...
			echo anchor_popup('mhs/simkrs_ksp/browse_dpa','[..]',$atrpop);
		}
...
<script language="javascript">
	function tanya(idkrs, kodemk){
		if(confirm('KONFIRMASI\nTekan OK untuk melanjutkan penghapusan data')){
			show("mhs/simkrs_ksp/delete2/"+idkrs+"/"+kodemk,"#center-column");
		}else{
			return false;
		}
	}
