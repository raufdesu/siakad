<head>
	...
	<script>
		function submitChangeStatus(){
			...
			load('admin/simcalonmhs/change_prodi/'+selected_prodi,'#center-column');
		}
	</script>
</head>
...
<?php
	...
	'url'=>site_url('admin/simcalonmhs/cari_calonmhs'), 'update'=>'#center-column', 'name'=>'pilih', 'id'=>'masmahasiswa',	'type'=>'post'));
...
<?php
	echo $this->pquery->form_remote_tag(array(
	'url'=>site_url('admin/simcalonmhs/simpan_nim'), 'update'=>'#center-column', 'name'=>'pilih', 'id'=>'masmahasiswa',	'type'=>'post'));
?>
	<table class="listing form" cellpadding="0" cellspacing="0">
		<tr>
			...
			<th style="width:88px">NIM <input type="submit" value="&nbsp;" class="save" name="cmdsimpan" /></th>
...
?>
	<tr>
		...
		<td class="first">
			<input type="hidden" name="nodaft<?php echo $x?>" value="<?php echo $bm['no_daftar']?>" />
			<input type="text" tabindex="<?php echo $x?>" <?php if($bm['status']) echo 'readonly'; ?> name="nim<?php echo $x?>" value="<?php echo $bm['nim']?>" size="10" />
		</td>
		<td style="tex-align:center;" class="first">
			...
			<a href="javascript:void(0)" title="Hapus Data CAMABA" onclick='return tanya(<?php echo $bm['no_daftar']?>)'>
				<?php echo img('asset/images/design/hr.gif')?>
			</a>
			<?php if($bm['status'] != '1'){ ?>
			<!--<a href="javascript:void(0)" title="Jadikan Sebagai Mahasiswa" onclick='show("admin/simcalonmhs/insert_nim/</?php echo $bm['no_daftar'].'/'.$this->session->userdata('sesi_nimdaftar');?>","#center-column")'>
				</?php echo img('asset/images/design/detail_add.png')?>
			</a>-->
			<?php }else{ ?>
			<a href="javascript:void(0)" title="Sudah Menjadi Mahasiswa">
