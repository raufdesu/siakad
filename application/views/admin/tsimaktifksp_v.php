<head>
	...
	<script>
		...
		function ChangeStatusTugas(){
			...
			load('admin/simaktifksp/change_status/'+selected_status,'#center-column');
		}
...
<div class="top-bar-adm">
	<div style="float:right;margin-right:-32px">
		<a href="javascript:void(0)" class='button' onclick='show("admin/simaktifksp/input","#center-column")'>Input</a>
		<a href="javascript:void(0)" class='button' onclick='show("admin/simaktifksp/akumulasi","#center-column")'>Rekapitulasi</a>
	</div>
	<h1>Mahasiswa Aktif KSP</h1>
...
<?php
	...
	'url'=>site_url('admin/simaktifksp/cari'), 'update'=>'#center-column', 'name'=>'cari', 'id'=>'simaktifksp', 'type'=>'post'));
	echo "<label>".form_input("txtCari",$this->session->userdata("cari_simaktifksp"),'size=30')."</label>";
...
<script language="javascript">
	function tanya(nim, status){
		if(confirm("KONFIRMASI\nTekan OK Untuk Menjadikan Status "+status+" Untuk Bagi "+nim)==true){
			show("admin/simaktifksp/status/"+nim+"/"+status,"#center-column");
			return true;
		}else{
			return false;
		}
	...
	function tanyahapus(nim, thajaran){
		if(confirm("KONFIRMASI\nTekan OK untuk penghapusan data\nmahasiswa dengan NIM : "+nim)==true){
			show("admin/simaktifksp/hapus/"+nim+"/"+thajaran, "#center-column");
			return true;
		}else{
			return false;
		}
