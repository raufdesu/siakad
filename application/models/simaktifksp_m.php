<?php
	Class Simaktifksp_m extends Model{
		...
		function get_bystatus($limit1, $limit2, $prodi = '', $thajaran = '', $status = ''){
			...
			$sql = "SELECT mhs.nim, nama, kodeprodi kdprod,
						...
						(SELECT status FROM simaktifksp WHERE nim = mhs.nim AND thajaran = '".$thajaran."') AS status
					FROM masmahasiswa mhs LEFT JOIN simaktifksp aksem ON mhs.nim=aksem.nim WHERE
					aksem.thajaran = '".$thajaran."' AND 
					mhs.nim IN (SELECT nim FROM simaktifksp WHERE thajaran = '".$thajaran."') ";
			if($prodi){
			...
			if($this->session->userdata('cari_simaktifksp')){
				/*$sql .= " AND nama LIKE '%".$this->session->userdata('cari_simaktifksp')."%' "; */
				$sql .= " AND mhs.nim LIKE '%".$this->session->userdata('cari_simaktifksp')."%' ";			
			}
				$sql .= " LIMIT ".$limit1;
			if($limit2 == true){
				$sql .= " ,".$limit2;
		...
		function nim_aktifsemester($thajaran, $status){
			$data = array();
			$sql = "SELECT nim FROM simaktifksp WHERE thajaran = '".$thajaran."' AND status = '".$status."'";
			$hasil = $this->db->query($sql);
			if($hasil->num_rows() > 0){
				$data = $hasil->result();
			}
		...
		function export_toaktifsemester(){
			$output = '';
			$sql_mhs = "SELECT masmahasiswa.nim, simaktifksp.status, masmahasiswa.statusakademik FROM masmahasiswa, simaktifksp  WHERE statusakademik ='Belum Lulus' and masmahasiswa.nim = simaktifksp.nim group by masmahasiswa.nim";
		...
		function insert_one(){
			...
			$this->db->insert("simaktifksp",$data);
		}
		...
		function select_mhs($limit1, $limit2, $prodi = '', $thajaran = ''){
			...
			$sql .= " (SELECT jenjang FROM simprodi WHERE kodeprodi = kdprod) AS jenjang,
						(SELECT namaprodi FROM simprodi WHERE kodeprodi = kdprod) AS nama_prodi ";
			$sql .= " FROM masmahasiswa mhs INNER JOIN simaktifksp aksem ON mhs.nim=aksem.nim WHERE
					aksem.thajaran = '".$thajaran."' ";
			/* $sql .= " AND mhs.nim IN (SELECT nim FROM simaktifksp WHERE thajaran = '".$thajaran."') "; */
			if($prodi){
				$sql .= " AND kodeprodi = ".$prodi." ";
			}
			...
			if($this->session->userdata('cari_simaktifksp')){
				$sql .= " AND (nama LIKE '%".$this->session->userdata('cari_simaktifksp')."%' ";
				$sql .= " OR mhs.nim LIKE '%".$this->session->userdata('cari_simaktifksp')."%') ";
			}
			$sql .= " ORDER BY nim";
			$sql .= " LIMIT ".$limit1;
			if($limit2 == true){
		...
		function count_all_mhs($prodi = '', $thajaran = ''){
			...
			$sql = "SELECT mhs.nim, nama, kodeprodi kdprod
					FROM masmahasiswa mhs LEFT JOIN simaktifksp aksem ON mhs.nim=aksem.nim WHERE
					aksem.thajaran = '".$thajaran."'
					AND mhs.nim IN(SELECT nim FROM simaktifksp WHERE thajaran = '".$thajaran."')";
			if($prodi){
				$sql .= " AND kodeprodi = ".$prodi." ";
			}
			if($this->session->userdata('sesi_statussem')){
			...
			if($this->session->userdata('cari_simaktifksp')){
				$sql .= " AND (mhs.nim LIKE '%".$this->session->userdata('cari_simaktifksp')."%' ";
				$sql .= " OR nama LIKE '%".$this->session->userdata('cari_simaktifksp')."%') ";
			}
		...
		function statuskan($nim, $status, $thajaran = ''){
			...
			$this->db->where('thajaran', $thajaran);
			$this->db->where("nim", $nim);
			$this->db->update("simaktifksp", $data);
		}
		/*
		function select_mk(){
			$data = array();
			$sql = "SELECT *,(SELECT namamk FROM simnamamk nm WHERE nm.kodenama = kur.kodenama) AS nama_mk FROM simkurikulum kur WHERE 
					kur.kodemk NOT IN
					(SELECT tawar.kodemk FROM simaktifksp tawar)";
		...
		function detail($kodemk){
			...
			$this->db->select("*");
			$this->db->from("simaktifksp");
			$this->db->join("simnamamk","simaktifksp.kodenama = simnamamk.kodenama");
			$this->db->join("simprodi","simaktifksp.kodeprodi = simprodi.kodeprodi");
			$this->db->where("kodemk",$kodemk);
			$hasil = $this->db->get();
			if($hasil->num_rows() > 0){
				$data = $hasil->result();
