<?php
	Class Simkrs_ksp extends Controller{
		...
		function input($data=''){
			...
			$data['cekwaktuksp'] = $this->simkrs_m->cek_waktuksp();
			...
			$this->load->view('mhs/isimkrs_k', $data);
		}
		function browse_dpa(){
			...
			if($cekquota == false){
				if($ceksama == false){
					...
					if($this->input->post('txt_nama_mk')){
						echo '<div class="error-top">Matakuliah '.$this->input->post('txt_nama_mk').' belum di ambil di semester sebelumnya</div>';
					}
				}
					$data['nama_matkul'] = '';
