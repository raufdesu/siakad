<?php
	Class Simmatrikulasi extends Controller{
		...
		function listview(){
			...
			// $data['total_page']	= $this->db->count_all('simmatrikulasi');
			$data['total_page']	= $this->simmatrikulasi_m->count_allbyprodi($kodeprodi, $this->session->userdata('sesicari_mhsmatrik'));
			$perpage	= 10;
			$data3		= $this->simpliparse->getAjaxPagination($data['total_page'],
				$perpage,'prodi/simmatrikulasi/listview/',4,'#center-column');
			$data['paging'] = $data3['paging'];
			$data["browse_mhsmatrikulasi"] = $this->simmatrikulasi_m->get_allbyprodi($data['no'], $perpage, $kodeprodi, $this->session->userdata('sesicari_mhsmatrik'));
			$this->load->view("prodi/simmatrikulasi/tsimmatrikulasi_v",$data);
		}
		function input_awal(){
			$atrs = array(
				'sesi_krs_nim' => '',
		...
		function input(){
			...
			$this->load->view("prodi/simmatrikulasi/isimmatrikulasi_v",$data);
		}
		function detail_mahasiswa(){
			...
			$cek_nimprodi = $this->simprodi_m->cek_nimprodi($this->session->userdata('sesi_prodi'), $nim);
			if($cek_nimprodi){
				...
				$namaprodi = $this->simprodi_m->get_namaprodi($this->session->userdata('sesi_prodi'));
				echo $this->simplival->alert('KONFIRMASI\nHarap memasukkan NIM PRODI '.$namaprodi);
			}
		}
		function edit(){
			$data['browse_prodi']	= $this->simprodi_m->select('','');
			$data["detail_simkurikulum"] = $this->simkurikulum_m->detail($this->uri->segment(4,0));
			$this->load->view("prodi/esimkurikulum_v",$data);
		}
		function save(){
			...
			if ($this->form_validation->run() == FALSE){
				...
				if($cek_kodemk == true){
					...
					redirect("prodi/simmatrikulasi/input");
				}
			}
		}
		function delete(){
