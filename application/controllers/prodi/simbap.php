...
Class Simbap extends Controller{
	...
	function daftarmatkul_bydosen(){
		...
		$this->load->view('prodi/simbap/tpresensi_minus_v', $data);
	}
	...
	function input($id_kelas){
		...
		$this->load->view('prodi/simbap/isimbap_v', $data);
	}
	function edit($idbap = ''){
		if(!$idbap){
		...
		$this->load->view('prodi/simbap/esimbap_v', $data);
	}
	...
	function input_presensi(){
		...
		if($sesi_dosenampu){
			...
			$this->load->view('prodi/simbap/isimpresensimhs_v', $data);
		}
	}
	function save_presensi(){
		...
		$this->load->view('prodi/simbap/tsukses_simpan_v', $data);
	}
	function cetak(){
		...
		$this->load->view('prodi/simbap/cbap_v', $data);
	}
	function update(){
		...
		redirect('prodi/simbap/browse/'.$this->input->post('id_kelas_dosen'));
	}
	function save(){
		...
		if($this->form_validation->run() == FALSE){
			...
			redirect('prodi/simbap/input_presensi');
		}
	}
	function hapus(){
		$this->simbap_m->delete($this->uri->segment(4));
		redirect('prodi/simbap/browse/'.$this->session->userdata('sesi_dosenampu'));
	}
}
?>