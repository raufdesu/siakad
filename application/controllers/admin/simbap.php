...
Class Simbap extends Controller{
	...
	function daftarmatkul_bydosen(){
		...
		$this->load->view('admin/simbap/tpresensi_minus_v', $data);
	}
	...
	function input($id_kelas){
		...
		$this->load->view('admin/simbap/isimbap_v', $data);
	}
	function edit($idbap = ''){
		if(!$idbap){
		...
		$this->load->view('admin/simbap/esimbap_v', $data);
	}
	...
	function input_presensi(){
		...
		if($sesi_dosenampu){
			...
			$this->load->view('admin/simbap/isimpresensimhs_v', $data);
		}
	}
	function save_presensi(){
		...
		$this->load->view('admin/simbap/tsukses_simpan_v', $data);
	}
	function cetak(){
		...
		$this->load->view('admin/simbap/cbap_v', $data);
	}
	function update(){
		...
		redirect('admin/simbap/browse/'.$this->input->post('id_kelas_dosen'));
	}
	function save(){
		...
		if($this->form_validation->run() == FALSE){
			...
			redirect('admin/simbap/input_presensi');
		}
	}
	function hapus(){
		$this->simbap_m->delete($this->uri->segment(4));
		redirect('admin/simbap/browse/'.$this->session->userdata('sesi_dosenampu'));
	}
}
?>