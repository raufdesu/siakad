<?php
	Class Masmahasiswa extends Controller{
		function __construct(){
			parent::Controller();
			$this->load->model(array('masmahasiswa_m','simprodi_m'));
		...
		function cetak(){
			$prodi = $this->session->userdata('sesi_prodimhs');
			$angkatan = $this->session->userdata('sesi_angkatanmhs');
			$data['title'] = 'Data Mahasiswa '.$this->auth->get_namaprodi($this->session->userdata('sesi_prodimhs'));
			...
			$this->load->view('admin/masmahasiswa/cmasmahasiswa_v', $data);
		}
		function listview(){
			$prodi = $this->session->userdata('sesi_prodimhs');
			$angkatan = $this->session->userdata('sesi_angkatanmhs');
		...
		/*function listview(){
			...
			$data['browse_prodi'] = $this->simprodi_m->select();
			$data['browse_angkatan'] = $this->masmahasiswa_m->get_disangkatan();
			$data['total_page']	= $this->masmahasiswa_m->count_all($prodi);
			$perpage	= 10;
			$data3		= $this->simpliparse->getAjaxPagination($data['total_page'],
				$perpage,'admin/masmahasiswa/listview/',4,'#center-column');
			$data['paging'] = $data3['paging'];
			$data["browse_masmahasiswa"] = $this->masmahasiswa_m->select($data['no'], $perpage, $prodi);
			$this->load->view("admin/tmasmahasiswa_v",$data);
		} */
		...
		function edit($id = ''){
			...
			$this->load->view("admin/emasmahasiswa_v",$data);
		}
		function save(){
			$config = array(
					array(
		...
		function detail(){
			...
			$this->load->view("admin/tdmasmahasiswa_v",$data);
		}
