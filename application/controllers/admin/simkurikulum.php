<?php
	Class Simkurikulum extends Controller{
		...
		function thn_kurikulum(){
			...
			redirect('admin/simkurikulum/listview');
		}
		function index(){
		...
		function listview(){
			...
			$data['total_page']	= $this->simkurikulum_m->count_all();
			$perpage	= 10;
			$data3		= $this->simpliparse->getAjaxPagination($data['total_page'],
				$perpage,'admin/simkurikulum/listview/',4,'#center-column');
			$data['paging'] = $data3['paging'];
			$data["browse_simkurikulum"] = $this->simkurikulum_m->select($data['no'],$perpage);
			$this->load->view("admin/simkurikulum/tsimkurikulum_v",$data);
		}
		function add(){
			if($this->uri->segment(4,0)){
				$this->session->set_userdata('sesi_thnkurikulum', $this->uri->segment(4,0));
			...
			$this->load->view("admin/simkurikulum/isimkurikulum_v",$data);
		}
		function edit(){
			$data['browse_prodi']	= $this->simprodi_m->select('','');
			$data["detail_simkurikulum"] = $this->simkurikulum_m->detail($this->uri->segment(4,0));
			$this->load->view("admin/simkurikulum/esimkurikulum_v",$data);
		}
		function save(){
		...
		function detail(){
			$data["detail_simkurikulum"] = $this->simkurikulum_m->detail($this->uri->segment(4,0));
			$this->load->view("admin/tdsimkurikulum_v",$data);
		}
		function popdetail(){
			$data["detail_simkurikulum"] = $this->simkurikulum_m->detail($this->uri->segment(4,0));
			$this->load->view("admin/simkurikulum/tdsimkurikulum_v", $data);
		}
