<?php
	Class Simmktawar extends Controller{
		...
		function ganti_kelas(){
			...
			$this->pengambilmk();
			redirect('admin/simmktawar/pengambilmk');
		}
		function export_pengambilmk(){
			...
			$this->load->view('admin/expengambilmk_v', $data);
		}
		function pengambilmk(){
			...
			if($this->uri->segment(5)){
				...
				$this->session->set_userdata('sesi_namamatkul', $this->uri->segment(5));
				// $asr = array(
			...
			// if($this->uri->segment(3,0)){
				...
				$this->load->view('admin/tpengambilmk_v', $data);
			// }
		}
		function presensi(){
		...
		function listpresensi(){
			...
			$data['total_page'] = $this->simdosenampu_m->count_matkul($this->session->userdata('sesi_thajaran'), $cari);
			...
			$data3		= $this->simpliparse->getAjaxPagination($data['total_page'],
				$perpage,'admin/simmktawar/listpresensi/',4,'#center-column');
			$data['paging'] = $data3['paging'];
			$data['browse_mkdosenampu'] = $this->simdosenampu_m->get_matkul($perpage,$data['no'],$this->session->userdata('sesi_thajaran'), $cari);
			$data["title"]	= "Daftar Matakuliah";
			$this->load->view("admin/tpresensi_v",$data);
		}
		function gantikelas(){
			if($this->uri->segment(4)){
				$this->session->set_userdata('sesi_kelas', $this->uri->segment(4,0));
				redirect('admin/simmktawar/mhsambilmk');
			}
		}
		function mhsambilmk(){
			if($this->uri->segment(4)){
		...
		function cetak_coverpresensi(){
			...
			$data['hari'] = $head['hari'];
			$data['ruang'] = $head['ruang'];
			$data['jamawal'] = $head['jamawal'];
			$data['jamselesai'] = $head['jamselesai'];
			...
			$this->load->view('admin/ccover_presensimhs_v', $data);
		}
		...
		function export(){
			...
			$data['ampu'] = $this->simdosenampu_m->get_one($id_kelas_dosen);
			$this->load->view('admin/simambilmk/expengambilmk_v', $data);
		}
		...
		function thn_mktawar(){
			...
			redirect('admin/simmktawar/listview');
		}
		function update_quota(){
			$this->simmktawar_m->update_quota($this->input->post('kode'));
			redirect('admin/simmktawar/listview/'.$this->input->post('segment'));
		}
		function index(){
			$this->_empty_sesi();
			redirect('admin/simmktawar/listview');
		...
		function listview(){
			...
			$data['browse_thn_ajaran'] = $this->simsetting_m->select();
			/* $data['browse_mk'] = $this->simmktawar_m->select_mk(); */
			$data['total_page'] = $this->simmktawar_m->count_tawar();
			/* $data['total_page']	= $this->simmktawar_m->count_all(); */
			$data['no']	= $this->uri->segment(4,0);
			$perpage	= 10;
			$data3		= $this->simpliparse->getAjaxPagination($data['total_page'],
				$perpage,'admin/simmktawar/listview/',4,'#center-column');
			$data['paging'] = $data3['paging'];
			$cari = $this->session->userdata('sesicari_matakuliahtawar');
			$data['browse_mktawar'] = $this->simmktawar_m->get_tawar($perpage,$data['no'],$this->session->userdata('sesi_thajaran'),$cari);
			$data["title"]	= "Daftar Matakuliah Yang Ditawarkan";
			$this->load->view("admin/tsimmktawar_v",$data);
		}
