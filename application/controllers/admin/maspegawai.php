<?php
	Class Maspegawai extends Controller{
		...
		function listview(){
			...
			$this->load->view("admin/tmaspegawai_v",$data);
		}
		function add(){
			...
			$this->load->view("admin/maspegawai/imaspegawai_v",$data);
		}
		function browse_dosen(){ // BROWSE UNTUK MEMILIH KETUA PRODI
			...
			$data['paging'] = $data3['paging'];
			$data["browse_maspegawai"] = $this->maspegawai_m->select($data['no'], $perpage);
			$this->load->view("admin/maspegawai/bmaspegawai_v",$data);
		}
		function edit($id = ''){
			if(!$id){
			...
			$this->load->view("admin/maspegawai/emaspegawai_v",$data);
		}
		function save(){
			$config = array(
					array(
