...
class Auth {
	...
	function get_thactive(){
		$this->CI->db->from('simsetting');
		$this->CI->db->where('aktif', 'Aktif');
	...
	function get_angkatanbynim($nim){
		$this->CI->db->select('angkatan');
		$this->CI->db->from('masmahasiswa');
		$this->CI->db->where('nim', $nim);
	...
	function namadekan(){
		$sql = "SELECT nppdekan,
				(SELECT nama FROM maspegawai WHERE npp=simprofil.nppdekan)namadekan
				FROM simprofil";
	...
	function namaakad(){
		$sql = "SELECT nppakad,
				(SELECT nama FROM maspegawai WHERE npp=simprofil.nppakad)namaakad
				FROM simprofil";
		$hasil = $this->CI->db->query($sql);
		return $hasil->row();
	}
	function get_profil(){
		$this->CI->db->from('simprofil');
		$this->CI->db->join('tkabupaten', 'simprofil.idkabupaten=tkabupaten.idkabupaten');
		$hasil = $this->CI->db->get();
		if($hasil->num_rows() > 0){
			return $hasil->row();
		}else{
	...
	function get_emaildpa($nim){
		$sql = "SELECT email FROM maspegawai WHERE npp = (SELECT npp FROM simdosenwali WHERE nim = '".$nim."')";
		$query = $this->CI->db->query($sql);
		return $query->email;
	}
	function get_namamhsbynim($nim){
		$this->CI->db->select('nama');
		$this->CI->db->from('masmahasiswa');
		$this->CI->db->where('nim', $nim);
		$query = $this->CI->db->get();
	...
	function get_prodi($kodeprodi){
		$sql = "SELECT *,
				(SELECT nama FROM maspegawai WHERE npp=simprodi.npp)kaprodi,
				(SELECT nip FROM maspegawai WHERE npp=simprodi.npp)nip,
				(SELECT nidn FROM maspegawai WHERE npp=simprodi.npp)nidn
				FROM simprodi WHERE kodeprodi = '".$kodeprodi."'";
		$hasil = $this->CI->db->query($sql);
		return $hasil->row();
	}
	function checkIP()
	...
	function get_prodibynim($nim){
		$sql = "SELECT *,
			(SELECT nama FROM maspegawai WHERE npp = simprodi.npp )kaprodi,
			(SELECT npp FROM maspegawai WHERE npp = simprodi.npp )nppkaprodi
			FROM simprodi WHERE kodeprodi = (SELECT kodeprodi FROM masmahasiswa WHERE nim = '".$nim."')";
		$hasil = $this->CI->db->query($sql);
		return $hasil->row();
	}
	function get_namaprodi($kode = ''){
		$sql = "SELECT namaprodi FROM simprodi WHERE kodeprodi = '".$kode."'";
		$query = $this->CI->db->query($sql);
		if($query->num_rows() > 0){
			return $query->row()->namaprodi;
		}else{
			return false;
		}
	}
	function get_namafakultasbykodeprodi($kodeprodi = ''){
		$sql = "SELECT namafakultas FROM simfakultas INNER JOIN simprodi
				ON simfakultas.kodefakultas = simprodi.kodefakultas
				WHERE kodeprodi = '".$kodeprodi."'";
		$query = $this->CI->db->query($sql);
		if($query->num_rows() > 0){
			return $query->row()->namafakultas;
		}else{
			return false;
		}
	}
	function get_namaprodibynim($kode = ''){
		$sql = "SELECT namaprodi FROM simprodi WHERE kodeprodi = '".$kode."'";
		$query = $this->CI->db->query($sql);
		if($query->num_rows() > 0){
			return $query->row()->namaprodi;
	...
	function get_kodeprodibynim($nim){
		...
		$query = $this->CI->db->query($sql);
		if($query->num_rows() > 0){
			return $query->row()->kodeprodi;
		}else{
			return false;
		}
	}
	function get_prefnamaprodi($pref = ''){
		$sql = "SELECT namaprodi FROM simprodi WHERE prefkodeprodi = '".$pref."'";
		$query = $this->CI->db->query($sql);
		if($query->num_rows() > 0){
			return $query->row()->namaprodi;
		}else{
